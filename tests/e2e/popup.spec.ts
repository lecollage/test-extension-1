...
test('renders the popup and updates the counter across loads', async ({ extensionId, page }) => {
  await page.goto(`chrome-extension://${extensionId}/popup.html`);

  const openDetailsButton = page.getByRole('button', { name: 'Open time details' });
  ...
  await expect(page.getByRole('heading', { name: 'Time Popup Counter' })).toBeVisible();
